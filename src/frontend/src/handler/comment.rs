...
use risingwave_sqlparser::ast::{CommentEntry, CommentObject, ObjectName};
...
/// Build a single `PbComment` from comment entry components.
fn build_comment(
    session: &SessionImpl,
...
pub async fn handle_comment(
    ...
    // Build all comments first (validates all entries before making any changes)
    ...
    catalog_writer.comment_on(comments).await?;
